...
library(tree)
new_tree <- tree(Carbon_Sequestration_lb ~ Tree_Height + Crown_Width_EW, data = my_pdxTrees )
set.seed(100)
new_preds <- predict(new_tree, my_pdxTrees_test)

my_tree_cv<-cv.tree(new_tree)
...
```{r}
Full_Tree_MSE <- mean((new_preds - my_pdxTrees_test$Carbon_Sequestration_lb)^2, na.rm = T)
data.frame(Full_Tree_MSE)
```


```{r}
prune_my_tree<-prune.tree(new_tree , best = 5)
small_tree_preds<-predict(prune_my_tree, newdata = my_pdxTrees_test)
small_Tree_MSE <- mean((small_tree_preds - my_pdxTrees_test$Carbon_Sequestration_lb)^2, na.rm = T)
...
```{r}
par(mfrow=c(1,2))
plot(my_pdxTrees_test$Tree_Height, my_pdxTrees_test$Crown_Base_Height, pch=19, xlab="Tree Height", ylab = "Crown Height", cex = .5, col = "darkgrey")
partition.tree(prune_my_tree, label="Species", add=TRUE)


plot(my_pdxTrees_test$Tree_Height, my_pdxTrees_test$Crown_Base_Height, pch=19, xlab="Tree Height", ylab = "Crown Height", cex = .5, col = "darkgrey")
partition.tree(new_tree, label="Species", add=TRUE)
```